...
IMAGE_NAME="mc-hybrid-processor:latest"
CONTAINER_NAME="my-mc-hybrid-processor-instance"
...
if [ $? -ne 0 ]; then
    echo "Error: Docker image build failed for '$IMAGE_NAME'. Please check the Dockerfile and logs."
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Error: Failed to run new container '$CONTAINER_NAME'. Check Docker logs for more details."
    exit 1
fi
